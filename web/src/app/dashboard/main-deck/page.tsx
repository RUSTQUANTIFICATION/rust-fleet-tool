...
export default function MainDeckDashboardPage() {
  ...
  async function loadAll() {
    try {
      ...
      for (const p of photos) {
        ...
        const deck = s.area_name || "Main Deck";
        const location = p.location_tag || "(no tag)";
        const rust = rustMap.get(p.id) ?? 0;

        const key = `${thisVesselId}__${deck}__${location}`;
  ...
  return (
    <div style={{ padding: 18, maxWidth: 1100 }}>
      <h1 style={{ fontSize: 22, fontWeight: 800 }}>Main Deck Dashboard</h1>
      <p style={{ opacity: 0.8 }}>Top main deck locations by rust %.</p>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ textAlign: "left", background: "#fafafa" }}>
              ...
              <th style={{ padding: 10, borderBottom: "1px solid #eee" }}>Deck | Location</th>
            ...
            {filteredSummary.length === 0 && (
              <tr>
                <td colSpan={5} style={{ padding: 12, opacity: 0.7 }}>
                  No main deck data for the selected vessel.
                </td>
              </tr>
            )}
          </tbody>
